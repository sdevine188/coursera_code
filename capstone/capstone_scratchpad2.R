...
cname <- "C:/Users/Stephen/Desktop/R/coursera_code/capstone/en_US"
dir(cname)   
list.files(cname)

# create the corpus
docs <- Corpus(DirSource(cname), readerControl = list(reader = readPlain, language = 'en'))
docs
summary(docs)   
...
# combine corpus into one corpus
corpus <- c(twitter_corpus, blogs_corpus, news_corpus)

# exploratory analysis of corpus
inspect(docs[1]) # view meta data on first document in corpus
as.character(docs[[1]]) # view text of first document in corpus
...
news_four_gram_tdm <- TermDocumentMatrix(news_corpus2, control = list(tokenize = four_gramTokenizer))

# inspect four_gram tdm
inspect(news_four_gram_tdm)
inspect(news_four_gram_tdm[1000:1050, 1:10])
news_four_gram_tdm_row_sums <- as.matrix(slam::row_sums(news_four_gram_tdm, na.rm=TRUE))
# convert to a dataframe
news_four_gram_freq <- data.frame(term = rownames(news_four_gram_tdm_row_sums), freq = news_four_gram_tdm_row_sums[, 1])
rownames(news_four_gram_freq) <- NULL
news_four_gram_freq <- arrange(news_four_gram_freq, desc(freq))
head(news_four_gram_freq)
...
# quiz 2
# " a case of"
q1_trigram <- news_trigram_freq %>%
        filter(term == "a case of")
head(q1)

q1_four_gram <- grep("a case of", news_four_gram_freq$term)
q1_freq <- news_four_gram_freq[q1_four_gram, ]
